...
export class TripDbService {
  ...
  getTripById(tripId: number): Promise<Trip> {
    ...
    return new Promise(async (resolve, reject) => {
      (await getConnection()).query(sql, function (error: any, result: any) {
        resolve(new Trip(result[0].id, result[0].country, result[0].countryId, result[0].city, result[0].arrivalDate, result[0].departureDate, result[0].status, result[0].userId));
      });
    });
  }
    ...
    return new Promise(async (resolve, reject) => {
      (await getConnection()).query(sql, function (error: any, result: any) {
        ...
        for (let i = 0; i < result.length; i++) {
          trips.push(new Trip(result[i].id, result[i].country, result[i].countryId, result[i].city, result[i].arrivalDate, result[i].departureDate, result[i].status, result[i].userId));
        }
        resolve(trips);
      });
    });
  ...
  getTrips() {
    ...
    return new Promise(async (resolve, reject) => {
      (await getConnection()).query(sql, function (error: any, result: any) {
        let trips: Trip[] = [];
        for (let i = 0; i < result.length; i++) {
          trips.push(new Trip(result[i].id, result[i].country, result[i].countryId, result[i].city, result[i].arrivalDate, result[i].departureDate,
  ...
  addTrip(trip: Trip) {
    const sql = mysql.format("INSERT INTO Trip (country,countryId,city,arrivaldate,departuredate,status,userId) VALUES(?,?,?,?,?,?,?)", [trip.getCountry(), trip.getCountryId(), trip.getCity(), trip.getArrivalDate(), trip.getDepartureDate(), trip.getStatus(), trip.getUserId()]);
    return new Promise(async (resolve, reject) => {
  ...
  updateTrip(trip: Trip) {
    const sql = mysql.format("UPDATE Trip SET country = ?, countryId= ?, city = ?, arrivaldate = ?, departuredate = ?, status= ? WHERE id=?",
      [trip.getCountry(), trip.getCountryId(), trip.getCity(), trip.getArrivalDate(), trip.getDepartureDate(), trip.getStatus(), trip.getTripId()]);
    return new Promise(async (resolve, reject) => {
      (await getConnection()).query(sql, (error: any, result: string) => {
        if (error) {
          if (error.errno === 1062) {
